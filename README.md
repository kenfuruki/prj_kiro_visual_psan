...
- **料率TBLモジュールの自動判定**: プログラム内に「COPY VB7～」のVCOPY句があるプログラムを自動判定し、視覚的に強調表示
- **プログラム説明の自動抽出**: PROGRAM-ID直後のコメントからプログラム説明を抽出し、ツールチップに表示
- **実ステップ数によるサイズ調整**: PROCEDURE DIVISION以降の実ステップ数に応じてノードサイズを動的調整
- **動的CALL検出**: 変数を使った動的CALLを検出し、WORKING-STORAGE SECTIONやMOVE文から実際のプログラム名を解決
- **ファイル名フィルタ**: ファイル名の先頭N桁を指定（複数指定可能）して、特定のプログラム群のみを処理対象にできる
- **ステップ数集計機能**: 指定したトッププログラムから到達可能なすべてのプログラムのステップ数を集計
- **インタラクティブ操作**: マウスで拡大・縮小・移動が可能なネットワーク図
- **呼び出し順序の可視化**: プログラム内でのCALL順序（1番目、2番目…）をエッジラベルとして表示
- **AI向けデータ出力**: シンプルなテキスト形式で依存関係を出力し、AIによる解析を容易に
...
### 2. ファイル名フィルタの設定（オプション）

特定のプログラム群のみを処理対象にしたい場合、`analyze.py` の `FILE_PREFIX_FILTER` 変数を編集します。

```python
# 全ファイルを処理（デフォルト）
FILE_PREFIX_FILTER = None

# QB7で始まるファイルのみ処理（3桁指定）
FILE_PREFIX_FILTER = "QB7"

# QB70で始まるファイルのみ処理（4桁指定）
FILE_PREFIX_FILTER = "QB70"

# QB70またはQB71で始まるファイルのみ処理（複数指定）
FILE_PREFIX_FILTER = ["QB70", "QB71"]
```

### 3. ステップ数集計対象の設定（オプション）

特定のトッププログラムからのステップ数を集計したい場合、`analyze.py` の `TOP_PROGRAMS` 変数を編集します。

```python
# 1つのプログラムを指定
TOP_PROGRAMS = ["QB7000"]

# 複数のプログラムを指定（それぞれ個別に集計）
TOP_PROGRAMS = ["QB7000", "QB712345"]
```
...
- `offline_call_graph.html`: インタラクティブなネットワーク図（ブラウザで開く）
...
- 料率TBLモジュールの自動判定（プログラム内に「COPY VB7～」のVCOPY句が存在）
- NetworkXによる有向グラフの生成
...
- `QB71RC.cbl`: 職域＆e-Auto料率TBL保持プログラム（COPY VB7～のVCOPY句を含む）
...
### `call_graph_structure.txt`

AI向けのシンプルな依存関係リストです。

**形式:**
```
- [Caller] --calls [order]--> [Callee]
```

**特徴:**
- 最小限のトークン消費
- AIが理解しやすいシンプルな構造
- 料率TBLモジュールには `(RateTable)` タグ付き

### `step_count_report.txt`

指定したトッププログラムから到達可能なプログラムのステップ数集計レポートです。
...
|--------------|---------|-----|------|-------|------|------------|
| 料率TBLモジュール | プログラム内に「COPY VB7～」のVCOPY句が存在 | オレンジ (#FF9800) | 四角形 (box) | 実ステップ数に応じて可変 | 太枠 (4px) | 料率TBL (VCOPY) + プログラム説明 + 実ステップ数 |
...
- **スタイル**: 破線（太め）
- **ラベル**: `[1]`, `[2]`, `[3]` ... （ファイル内での呼び出し順序）
- **ツールチップ**: `呼出順: N番目 (動的CALL)`
- **例**: `CALL WS-PGM-NAME USING ...`
- **特徴**: WORKING-STORAGE SECTIONのVALUE句やMOVE文で変数に代入された実際のプログラム名を表示
